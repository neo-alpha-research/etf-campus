...
- **적용 대상**: 모든 시각적 멀티포맷 산출물(인스타그램 카드뉴스 1~6슬라이드, 스레드 인포그래픽, 대시보드 및 공유 이미지 등)의 최하단에는 반드시 아래의 공식 단일 풋터(법정 면책 + 브랜드 도메인)를 일관되게 적용한다:
  1. **법정 면책 문구 (자본시장법 제101조 준수)**:
     - 문안: `* 본 자료는 투자 판단을 돕기 위한 정보 제공용이며, 특정 종목의 매수·매도를 권유하지 않습니다.`
     - 스타일: 색상 `#475569` (Slate-600), 폰트 크기 최소 `24px` 이상, 폰트 굵기 `font-weight="800"` 이상, 중앙 정렬 (`text-anchor="middle"`).
  2. **ETF 캠퍼스 공식 브랜드 & 도메인 워터마크**:
     - 문안: `ETF 캠퍼스 | https://etf-campus.pages.dev`
     - 스타일: 색상 `#047857` (Emerald-700), 폰트 크기 `28px` 이상, 폰트 굵기 `font-weight="900"`, 중앙 정렬 (`text-anchor="middle"`), 자간 `letter-spacing="0.5"`.
  3. **시각적 정제 규칙**:
     - 불필요한 배경 박스나 그림자 음영을 일체 배제하고 순수 텍스트 2줄로 깨끗하고 안정감 있게 렌더링한다.
