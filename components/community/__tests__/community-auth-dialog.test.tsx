...
describe("CommunityAuthDialog Turnstile 단계 전환", () => {
  ...
  it("HTTP 200 헤더 수신 후 본문 읽기 중 타임아웃(408) 발생 시에도 저장을 차단하고 [다시 시도] 버튼을 노출한다", async () => {
    const onAuthenticated = vi.fn();
    const bodyTimeoutError = new Error("요청 시간이 초과되었습니다. 잠시 후 다시 시도해 주세요.") as Error & { status: number; code: string };
    bodyTimeoutError.status = 408;
    bodyTimeoutError.code = "TIMEOUT";
    ...
    mocks.communityFetch
      ...
      // 1st GET /profile throws 408 TIMEOUT while streaming body
      .mockRejectedValueOnce(bodyTimeoutError)
      // 2nd GET /profile (after clicking retry) succeeds
      .mockResolvedValueOnce({
        hasNickname: true,
        hasTermsConsent: false,
    ...
    // 본문 수신 중 타임아웃 발생 시에도 에러 메시지 노출 및 저장 버튼 비활성화 확인
    await waitFor(() => {
      expect(screen.getByRole("alert")).toBeInTheDocument();
      expect(screen.getByText("요청 시간이 초과되었습니다. 잠시 후 다시 시도해 주세요.")).toBeInTheDocument();
      expect(screen.getByRole("button", { name: "다시 시도" })).toBeInTheDocument();
