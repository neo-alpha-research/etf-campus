...
export function CommunityFeed() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/community/posts${query}`)
      .then(async (response) => {
    ...
    <div className="page-shell py-7 sm:py-10">
      <section className="rounded-3xl border border-brand-100 bg-gradient-to-br from-brand-50 via-white to-white px-5 py-7 shadow-sm sm:px-8 sm:py-10">
        ...
        <div className="mt-3 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl"><h1 className="text-3xl font-extrabold tracking-tight text-slate-950 sm:text-4xl">ETF 판단 기준을 함께 배우고 검증합니다</h1><p className="mt-3 text-sm leading-6 text-slate-600 sm:text-base">특정 ETF 추천이나 수익 경쟁이 아닌, 연금·상품 구조·공시·위험·서비스 신뢰도를 확인하는 커뮤니티입니다.</p></div>
        ...
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950"><p><span className="font-bold">운영 공지</span><span className="mx-2 text-amber-400">|</span>게시물은 공개로 읽을 수 있으며, 작성·댓글·신고는 이메일 인증 회원만 가능합니다. 매수·매도 강요, 목표가, 수익 보장, 리딩방·광고 유도는 허용하지 않습니다.</p><p className="mt-2 text-amber-900">처음 이용한다면 <Link className="font-bold underline decoration-amber-400 underline-offset-2" href="/guides/">학습용 예시 가이드</Link>와 <Link className="font-bold underline decoration-amber-400 underline-offset-2" href="/books/">읽기 경로</Link>를 참고해 주세요. 예시는 판단 기준을 익히기 위한 자료이며 개인별 결정을 대신하지 않습니다.</p></div>
      </section>
      ...
      <section className="mt-5" aria-live="polite">
        ...
        {status === "ready" && posts.length > 0 ? <div className="space-y-3">{posts.map((post) => <Link key={post.slug} href={`/community/${post.slug}/`} className="block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-md"><div className="flex items-center gap-2 text-xs font-bold text-brand-700"><span>{post.category.name}</span><span className="text-slate-300">·</span><span className="text-slate-500">{displayDate(post.createdAt)}</span></div><h2 className="mt-2 text-lg font-bold text-slate-950">{post.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{post.excerpt ?? post.bodyText}</p><div className="mt-4 flex items-center gap-3 text-xs text-slate-500"><span>{post.authorNickname}</span><span>댓글 {post.commentCount}</span></div></Link>)}</div> : null}
      </section>
