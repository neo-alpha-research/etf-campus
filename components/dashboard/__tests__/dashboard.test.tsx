...
describe("Dashboard", () => {
  ...
  it("일반 계좌 기본 화면은 일반형·1,000억원 이상 종목만 보여준다", () => {
    ...
    expect(screen.getByText("순자산 1,000억+ · 1종목")).toBeInTheDocument();
  ...
  it("500억과 전체 범위를 전환하고 소규모 ETF를 순자산 옆에 표시한다", () => {
    ...
    expect(screen.getByText("순자산 500억+ · 2종목")).toBeInTheDocument();
  ...
  it("긴 종목명과 분류·연금 정보를 검색하기 쉽게 분리한다", () => {
    ...
    expect(nameHeader).toHaveClass("md:w-[174px]", "text-center");
