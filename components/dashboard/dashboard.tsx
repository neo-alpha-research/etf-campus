          ...
          <table className="w-full border-collapse text-left text-sm md:min-w-[1180px] md:table-fixed"><caption className="sr-only">{copy.title} 목록과 기간별 가격 수익률</caption>
            <thead className="border-b-2 border-neutral-300 bg-neutral-100 text-[13px] font-extrabold text-neutral-700">
              <tr>
                ...
                <th className="sticky top-0 z-10 w-28 bg-neutral-100 px-4 py-3 text-right md:hidden" scope="col">{RETURN_PERIOD_LABELS[normalizedPeriod]} 수익률</th>
                <th aria-label="총보수, 단위 퍼센트" className="sticky top-0 z-10 w-16 bg-neutral-100 px-2 py-3 text-right md:hidden" scope="col"><UnitHeaderLabel label="총보수" unit="%" /></th>
                <th aria-label="종가, 단위 원" className="sticky top-0 z-10 hidden w-[5%] whitespace-nowrap bg-neutral-100 px-0 py-2 text-right md:table-cell" scope="col"><UnitHeaderLabel label="종가" unit="원" /></th>
                <th aria-label="거래대금, 단위 억원" className="sticky top-0 z-10 hidden w-[6%] whitespace-nowrap bg-neutral-100 px-1 py-2 text-right md:table-cell" scope="col"><UnitHeaderLabel label="거래대금" unit="억원" /></th>
                <th aria-label="순자산, 단위 억원" className="sticky top-0 z-10 hidden w-[6%] whitespace-nowrap bg-neutral-100 px-1 py-2 text-right md:table-cell" scope="col"><UnitHeaderLabel label="순자산" unit="억원" /></th>
                ...
                {periods.map((period) => <th aria-label={`${RETURN_PERIOD_LABELS[period]} 수익률, 단위 퍼센트`} className={`sticky top-0 z-10 hidden w-[4.25%] whitespace-nowrap px-0.5 py-2 text-right md:table-cell ${normalizedPeriod === period ? "bg-brand-100 text-brand-900" : "bg-neutral-100"}`} key={period} scope="col"><UnitHeaderLabel label={RETURN_PERIOD_LABELS[period]} unit="%" /></th>)}
                <th aria-label="총보수, 단위 퍼센트" className="sticky top-0 z-10 hidden w-[4.5%] whitespace-nowrap bg-neutral-100 px-0.5 py-2 text-right md:table-cell" scope="col"><UnitHeaderLabel label="총보수" unit="%" /></th>
                ...
                <th className="sticky top-0 z-10 hidden w-[4%] whitespace-nowrap bg-neutral-100 px-0.5 py-3 text-center md:table-cell" scope="col">연금</th>
              </tr>
            </thead>
              ...
              {virtualRows.map((virtualRow) => {
                ...
                return (
                  <tr className="transition-colors hover:bg-brand-50/50" data-index={virtualRow.index} key={etf.ticker} ref={rowVirtualizer.measureElement}>
                    ...
                    {periods.map((period) => <td className={`hidden px-1 py-2.5 text-right text-xs md:table-cell ${normalizedPeriod === period ? "bg-brand-50/60" : ""}`} key={period}><ReturnCell showUnit={false} value={etf.returns[period]} /></td>)}
                    <td className="tabular-nums hidden px-1 py-2.5 text-right text-xs text-muted md:table-cell">{(etf.ter * 100).toFixed(2)}</td>
