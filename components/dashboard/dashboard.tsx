  ...
  return (
    <main aria-labelledby="dashboard-title" className="page-shell flex-1 py-4 sm:py-6">
      ...
      <section aria-label="ETF 검색과 정렬" className="mt-3 rounded-2xl border border-brand-200 bg-brand-50/40 p-3 shadow-sm sm:p-4">
        ...
        >
          <div className="relative min-w-0 w-full flex-1 sm:w-[600px] sm:max-w-full sm:flex-none">
        ...
        <div className="overflow-x-auto min-[1244px]:overflow-x-visible">
          <table className={`md-period-table w-full border-collapse text-left text-sm md:w-[1180px] md:min-w-[1180px] md:table-fixed ${state.mode === "new" ? "is-new" : ""}`}><caption className="sr-only">{copy.title} 목록과 기간별 가격 수익률</caption>
            <thead className="border-b-2 border-neutral-300 bg-neutral-100 text-[13px] font-extrabold text-neutral-700">
              <tr>
                <th className="sticky top-0 z-10 hidden w-[48px] whitespace-nowrap bg-neutral-100 px-0 py-3 text-center md:table-cell" scope="col">종목코드</th>
                <th className="sticky top-0 z-20 min-w-[134px] w-[134px] bg-neutral-100 px-1.5 py-3 text-center md:left-0 md:w-[134px]" scope="col">종목명</th>
            ...
            <tbody className="divide-y divide-line">
              {visibleEtfs.map((etf) => <tr className="transition-colors hover:bg-brand-50/50" key={etf.ticker}>
                <td className="tabular-nums hidden w-[48px] whitespace-nowrap px-0 py-2.5 text-center text-xs text-muted md:table-cell">{etf.ticker}</td>
