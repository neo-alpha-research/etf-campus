...
export function EtfDetailClient({ etf, children }: Props) {
  const [activeTab, setActiveTab] = useState<"summary" | "compare">("summary");
  const { basket, mounted, removeEtf, clearBasket } = useCompareBasket();
    ...
    <div className="mt-4">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-line pb-px">
        <nav className="-mb-px flex gap-6" aria-label="상세 탭">
          ...
          >
            현재 ETF + 비교 {mounted ? <span className={`ml-1.5 rounded-full px-2 py-0.5 text-[11px] font-extrabold ${basket.length > 0 ? "bg-brand-100 text-brand-800" : "bg-neutral-100 text-neutral-500"}`}>{basket.length}개</span> : null}
          </button>
        </nav>
      </div>
      ...
      <div className="pt-6">
        {activeTab === "summary" ? children : (
          mounted ? <EtfCompareView basket={basket} mainEtf={etf} onRemove={removeEtf} /> : <div className="py-20 text-center text-sm text-muted">비교 테이블을 불러오는 중...</div>
        )}
      </div>
