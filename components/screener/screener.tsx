    ...
    <main className="page-shell flex-1 py-8 sm:py-12">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <aside aria-label="ETF 필터" className={`${filtersOpen ? "fixed inset-x-0 bottom-0 z-40 max-h-[82vh] overflow-y-auto rounded-t-3xl bg-surface p-5 shadow-2xl" : "hidden"} md:static md:block md:max-h-none md:rounded-2xl md:border md:border-line md:bg-neutral-50 md:p-5 md:shadow-none`}>
          ...
          <fieldset className="mt-5 border-b border-line pb-5"><legend className="text-sm font-extrabold">계좌유형</legend><label className="mt-3 flex cursor-pointer items-center gap-3 rounded-xl bg-brand-50 p-3 text-sm font-bold text-brand-800"><input checked={filters.pensionOnly} className="size-4 accent-brand-700" onChange={(event) => updateFilters({ ...filters, pensionOnly: event.target.checked })} type="checkbox" />연금 가능만</label></fieldset>
          ...
          <fieldset className="py-5"><legend className="text-sm font-extrabold">순자산 구간</legend><div className="mt-3 space-y-2">{AUM_RANGES.map((value) => <label className="flex items-center gap-2 text-sm text-muted" key={value}><input checked={filters.aumRanges.includes(value)} className="size-4 accent-brand-700" onChange={() => updateFilters({ ...filters, aumRanges: toggleValue<AumRange>(filters.aumRanges, value) })} type="checkbox" />{aumLabels[value]}</label>)}</div></fieldset>
