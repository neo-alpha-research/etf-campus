...
export function ProfessorHero({
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-brand-200/90 bg-gradient-to-br from-brand-50/80 via-surface to-brand-50/50 p-6 sm:p-8 shadow-sm">
      ...
      <div className="relative flex flex-col md:flex-row items-center md:items-start gap-5 sm:gap-6">
        {/* Professor Owl Avatar with Academic Badge */}
        <div className="relative shrink-0 flex flex-col items-center">
          <div className="relative size-24 sm:size-28 rounded-2xl overflow-hidden border-2 border-brand-500/30 shadow-md ring-4 ring-brand-100 bg-surface">
            <Image
              src="/images/professor_owl.jpg"
              alt="ETF 캠퍼스 수석 멘토 부엉이 교수"
              width={160}
              height={160}
          ...
          <span className="mt-2 inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-xs font-extrabold bg-brand-800 text-white shadow-2xs">
            <span>🎓</span> 부엉이 교수
          </span>
        </div>

        {/* Speech Bubble & Welcome Message (정중한 교수님 존댓말 톤) */}
        <div className="flex-1 text-center md:text-left space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-brand-100 text-brand-800 text-xs font-extrabold">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-strong break-keep leading-tight">
            ...
            <span className="text-brand-700 underline decoration-brand-300 decoration-wavy decoration-2 underline-offset-4">
              ETF 캠퍼스
            </span>에 오신 것을 진심으로 환영합니다!&rdquo;
          </h1>

          <p className="text-sm sm:text-base text-muted font-medium break-keep leading-relaxed">
            복잡한 금융 시장에서 길을 잃지 않도록 제가 직접 캠퍼스 투어를 안내해 드리겠습니다.
            먼저 <strong className="text-strong font-bold">캠퍼스 투어</strong>로 주요 시설을 둘러보신 후,
            <strong className="text-strong font-bold"> 10강 퀴즈</strong>를 완주하시고
            <span className="text-amber-800 font-bold"> 🎁 [연금 ETF 운용 체크리스트 PDF]</span>를 받아 가시기 바랍니다.
          </p>
        </div>
      </div>

      {/* Mode Navigation Tabs */}
      <div className="mt-6 pt-5 border-t border-brand-100/90 flex flex-col sm:flex-row items-center justify-between gap-3">
