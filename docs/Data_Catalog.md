...
|---|---|---|---|---|---|
| **ETF 일별시세, NAV, 상장좌수** | **한국거래소 (KRX Open API)** 단일 원천 | `update_daily_data.py`<br>`backfill_api.py` (로컬 CSV 재사용) | **거래일 익일 07:50~08:40**<br>(NHN Cloud n8n 5분 단위 스마트 프로빙) | 1,167개 ETF 일괄 수집 → Git CSV 및 Cloudflare D1 (`etf_daily`, `etf_prices`) 적재<br>※ 공공데이터포털(FSC)은 오전 10시 이후 지연으로 시세 수집에서 완전 제외 | 단일 원천 (SSOT) / 정상 |
...
3. **「조세특례제한법」**:
   - **제91조의18(개인종합자산관리계좌에 대한 과세특례)** 및 동법 시행령 제93조의4:
     - **전 종목 편입 허용**: 국내 금융투자업자가 취급하는 국내 증권시장 상장 ETF는 **1,167개 전 종목 100% 편입 가능** (`isa_eligible = "가능"`). 별도의 종목별 인가/제한 목록이 없으므로 퇴직연금과 같은 별도의 외부 대조 원장이 필요하지 않음.
     - **자산 배분 규제 없음**: 위험자산/안전자산 비율 제한 없음 (0~100% 자율 운용).
     - **절세 실익 중심 관리**: 계좌 밖에서 매매차익이 이미 비과세인 국내주식형(354개) 대비, 해외주식·채권·리츠·커버드콜 등 계좌 밖에서 15.4% 배당소득세가 과세되는 종목(813개)이 ISA의 손익통산, 비과세(200만/서민 400만), 9.9% 분리과세 실익이 큼.
     - **스크리너 개편 원칙**: 연 2,000만원 납입 한도를 가진 투자자를 위해 중개형 ISA 탭은 절세 실익이 큰 813개 종목만 선별하여 나열함.

#### 2. 감사 메타데이터 스펙
- **`pension_limit`**: `"100% (안전자산)"` (235종목) | `"70% (위험자산)"` (796종목) | `"불가"` (136종목)
- **`isa_eligible`**: `"가능"` (1,167종목 전원 적격, 조특법 제91조의18에 의해 전 종목 편입 허용)
- **`isa_tax_type`**: `"국내주식형"` (354종목) | `"기타"` (813종목)
- **`isa_tax_benefit`**: `"높음"` (813종목, 15.4% 절세 실익) | `"낮음"` (354종목, 국내주식형)
- **`isa_education_required`**: `"Y"` (72종목: 레버리지 62 + 인버스 2X 10 사전교육/기본예탁금 필요) | `"N"` (1,095종목)
