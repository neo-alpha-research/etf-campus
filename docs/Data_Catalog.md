...
| **퇴직연금 및 ISA 적격성** | **퇴직연금감독규정 제9조·제12조 및 조특법 제91조의18** 단일 룰 | `pension_regulatory_engine.py`<br>`phase0_merge_verify.py` | 시세 갱신 시 자동 평가 | 위험평가액 40% 초과 파생상품, 레버리지, 인버스 제외<br>※ 배율 기반 ISA 교육 대상 및 신뢰도 메타데이터 산출 (2026-09-04) | 법정 감독규정 / 정상 |
...
#### 2. 감사 메타데이터 스펙
- **`pension_limit`**: `"100% (안전자산)"` (237종목) | `"70% (위험자산)"` (794종목) | `"불가"` (136종목)
- **`isa_eligible`**: `"가능"` (1,167종목 전원 적격) | `"불가"` (0종목)
- **`isa_education_required`**: `"Y"` (72종목: 레버리지 62 + 인버스 2X 10) | `"N"` (1,095종목: 인버스 -1X 31 + 일반 1,064)
- **`pension_source`**: `"표본대조"` (공식 투자설명서 및 증권사 라인업 대조 5종목) | `"규칙기반추정"` (1,162종목) | `"증권사목록대조"`
- **`pension_confidence`**: `"높음"` (1,046종목) | `"보통"` (121종목: 잔여 합성 및 커버드콜 종목 - 증권사 최종 확인 권장 안내 부착) | `"낮음"` (0종목)
