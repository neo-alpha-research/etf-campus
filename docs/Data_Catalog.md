...
### 2-6. 신규 종목 ISIN 수집 방침 (2026-08-26 확정)
- **유일한 출처:** ISIN 식별자는 공공데이터포털 증권상품시세정보(15094806)의 isinCd만을 사용합니다. KRX상장종목정보(15094775)에는 ETF가 포함되지 않음이 확인되었습니다.
- **파생 및 생성 금지:** 추정이나 규칙 기반으로 ISIN이나 체크디지트를 임의 생성하는 것은 엄격히 금지됩니다. (잘못된 ISIN이 조용히 자리잡는 것을 방지)
- **보류 목록 관리:** KRX 일별시세에는 수록되었으나 FSC 증권상품시세정보에 아직 수록되지 않아 ISIN을 얻을 수 없는 신규 상장 종목은, FSC 수록 시점까지 파이프라인에서 보류되며 매 실행 로그에 누락 경과 일수와 함께 기록됩니다.

### 2-7. 승인된 금융위원회 API 4종 목록 (2026-08-26 기준)
| 데이터 번호 | 이름 | 승인일 | 용도 | ETF 포함 여부 | 이용허락범위 |
|---|---|---|---|---|---|
| 15094806 | 증권상품시세정보 | 2026-07-17 | ETF 종가, 순자산, ISIN 수집 | **포함됨** | 제한 없음 |
| 15094775 | KRX상장종목정보 | 2026-07-17 | (일반 주식용) | **미포함** | 제한 없음 |
| 15094792 | 펀드상품기본정보 | 2026-08-14 | 상장일 등 메타데이터 출처 후보 | 조사 중 | 제한 없음 |
| 15094807 | 지수시세정보 | 2026-08-26 | 기초지수 종가 (추적오차율용/지수전환 후보) | 해당 없음 | 제한 없음 |
