...
## [FM-016] OSMU Mobile Layout & Typography Budget Integrity (모바일 레이아웃 및 타이포그래피 예산 결함 차단)
- **발생 양상**: 스마트폰 화면(폭 390~430px) 환경에서 스레드/인스타그램 지표 라인이 28자를 초과하여 외톨이 줄바꿈(Orphan wrap)이 발생하거나, 첫 줄 헤더가 45자를 초과하여 모바일 피드에서 어절이 깨지거나, 인스타그램 캡션 첫머리가 75자 골든 존을 초과하여 핵심 결론이 접히거나, 본문에 줄바꿈 계산을 깨뜨리는 괄호`()` 및 유니코드 이모지가 포함되거나, 채널별 플랫폼 알고리즘에 맞지 않는 해시태그 구성 결함.
- **실제 관측 사례**: 2026-09-18 OSMU 텍스트 생성 시 테마 및 수급 문장이 86자 장문으로 생성되어 모바일 화면에서 3~4줄로 불규칙하게 꺾이며 끝 단어가 밀려났던 문제 및 스레드/인스타그램 간 채널별 해시태그 알고리즘 최적화 부재.
- **근본 원인**: 모바일 뷰포트 글자수 예산(Character Budget: 지표 28자, 헤더 45자, 캡션 골든존 75자) 및 플랫폼별(스레드 단일태그 vs 인스타그램 탐색 큐레이션 태그) 최적화 검증 파이프라인 부재.
- **방어 대책**:
  1. 스레드 및 인스타그램 템플릿에 3행 2열 대칭 파이프 그리드(행당 최대 28자), 첫 줄 45자 헤더, 상단 75자 골든 존 안착, 40~48자 단문 불릿 규격 강제.
  2. 기계적 1 vs 2 투표를 배제하고 3층 구조 고밀도 2줄 단문 CTA 적용.
  3. 채널별 차등화 해시태그 전략: 스레드는 메타 알고리즘 감점 방지 및 탐색 도달 극대화를 위해 최고 선호 단일 토픽 태그(`#ETF투자`)를 강제하고, 인스타그램은 검색·탐색 탭(Explore Tab) SEO 노출을 위해 5대 큐레이션 해시태그를 `#ETF투자` 최우선 순위(`#ETF투자 #ETF브리핑 #마켓브리핑 #재테크 #ETFCampus`)로 정렬 유지 (10개 이상 무분별한 스팸 태그는 차단).
  4. 린터(`check_osmu_mobile_layout_budget()`)를 통해 템플릿 및 생성물의 글자 수 예산과 무괄호·무이모지·채널별 해시태그 무결성을 기계적으로 자동 검증·차단.
- **자동 검사**: `scripts/lint_pipeline.py` -> `check_osmu_mobile_layout_budget()`
