...
import { checkProfileConfigured, sessionHeaders } from "../_lib/session";
...
export async function onRequestPost(context) {
  ...
  const [captchaError, emailLimit, ipLimit] = await Promise.all([
    verifyTurnstile(context, payload?.captchaToken, "community_password_login"),
    enforceDatabaseRateLimit(context, "password-login-email", email, 10, 600),
    enforceDatabaseRateLimit(context, "password-login-ip", context.request.headers.get("CF-Connecting-IP") ?? "unknown", 20, 600),
  ]);
