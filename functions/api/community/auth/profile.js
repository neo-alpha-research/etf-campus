...
export async function onRequestGet(context) {
  ...
  if (error) return errorResponse(503, "UNAVAILABLE", "?„ë¡œ???•ë³´ë¥?ë¶ˆëŸ¬?????†ìŠµ?ˆë‹¤.");
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (error) return error.code === "23505" ? errorResponse(400, "VALIDATION_ERROR", "?´ë? ?¬ìš© ì¤‘ì¸ ?‰ë„¤?„ì…?ˆë‹¤.") : errorResponse(503, "UNAVAILABLE", "?„ë¡œ?„ì„ ?€?¥í•  ???†ìŠµ?ˆë‹¤.");
    const profile = Array.isArray(data) ? data[0] : data;
  ...
  } catch (error) { return error instanceof CommunityValidationError ? errorResponse(400, "VALIDATION_ERROR", error.message) : errorResponse(503, "UNAVAILABLE", "?„ë¡œ?„ì„ ?€?¥í•  ???†ìŠµ?ˆë‹¤."); }
}
