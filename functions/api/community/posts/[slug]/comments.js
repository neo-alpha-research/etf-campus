...
export async function onRequestGet(context) {
  ...
  if (!validSlug(slug)) return errorResponse(404, "NOT_FOUND", "ê²Œì‹œë¬¼ì„ ì°¾ì„ ???†ìŠµ?ˆë‹¤.");
  ...
  try {
    ...
    return errorResponse(503, "UNAVAILABLE", "?“ê???ë¶ˆëŸ¬?????†ìŠµ?ˆë‹¤.");
  }
}
  ...
  try {
    ...
    if (error) {
      if (error.message.includes("member profile")) {
        return errorResponse(403, "FORBIDDEN", "?‰ë„¤???¤ì •???„ë£Œ???¸ì¦ ?Œì›ë§??“ê????‘ì„±?????ˆìŠµ?ˆë‹¤.");
      }
      if (error.message.includes("post not found")) return errorResponse(404, "NOT_FOUND", "ê²Œì‹œë¬¼ì„ ì°¾ì„ ???†ìŠµ?ˆë‹¤.");
      throw error;
    }
    ...
    return errorResponse(503, "UNAVAILABLE", "?“ê????€?¥í•  ???†ìŠµ?ˆë‹¤.");
  }
}
