...
function rpcError(error, action) { const message = error?.message ?? ""; if (message.includes("forbidden")) return errorResponse(403, "FORBIDDEN", `본인이 작성한 댓글만 ${action}할 수 있습니다.`); if (message.includes("not found")) return errorResponse(404, "NOT_FOUND", "댓글을 찾을 수 없습니다."); return errorResponse(503, "UNAVAILABLE", `댓글을 ${action}할 수 없습니다.`); }
...
export async function onRequestPatch(context) {
  ...
  } catch (error) { return error instanceof CommunityValidationError ? errorResponse(400, "VALIDATION_ERROR", error.message) : errorResponse(503, "UNAVAILABLE", "댓글을 수정할 수 없습니다."); }
}
