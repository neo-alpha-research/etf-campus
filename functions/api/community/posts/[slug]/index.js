...
export async function onRequestGet(context) {
  ...
  } catch { return errorResponse(503, "UNAVAILABLE", "게시물을 불러올 수 없습니다."); }
}
...
export async function onRequestPatch(context) {
  ...
  try {
    ...
    if (error) return mapRpcError(error, "수정") ?? errorResponse(503, "UNAVAILABLE", "게시물을 수정할 수 없습니다.");
    return jsonResponse({ post: { slug: Array.isArray(data) ? data[0]?.slug : data?.slug } });
  } catch (error) { return error instanceof CommunityValidationError ? errorResponse(400, "VALIDATION_ERROR", error.message) : errorResponse(503, "UNAVAILABLE", "게시물을 수정할 수 없습니다."); }
}
...
export async function onRequestDelete(context) {
  ...
  if (error) return mapRpcError(error, "삭제") ?? errorResponse(503, "UNAVAILABLE", "게시물을 삭제할 수 없습니다.");
