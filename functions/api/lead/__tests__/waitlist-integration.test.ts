...
describe("Lead Waitlist Real DB Integration & Operational Scenarios", () => {
  ...
  it("[서버 측 접수 차단 통합 검증] WAITLIST_INGRESS_ENABLED !== 'true'인 모든 경우('false', undefined, 공백, 오타) 503 반환 및 DB 저장이 일체 발생하지 않는다", async () => {
    const invalidIngressValues = ["false", undefined, "", "   ", "ture", "invalid"];

    for (const val of invalidIngressValues) {
      const shutoffEnv: WaitlistEnv = {
        ...testEnv,
        WAITLIST_INGRESS_ENABLED: val,
      };
