...
describe("Turnstile 및 OTP 속도 제한 보안 계약", () => {
  ...
  it("Preview 외 환경에서 테스트용 Turnstile 시크릿 키 및 클라이언트 토큰을 엄격히 거부한다", () => {
    expect(source).toContain('env.COMMUNITY_ENVIRONMENT !== "preview" && env.TURNSTILE_SECRET_KEY === CLOUDFLARE_TEST_SECRET_KEY');
    expect(source).toContain("!isPreview && CLOUDFLARE_TEST_TOKENS.has(token)");
