...
$old = ': (isCustom ? customStart : defaultDates.start);'
$new = ': (isCustom ? customStart : (defaultDates.start ?? ""));'
if (-not $text.Contains($old)) { throw 'Expected chart start range fallback was not found; no file was changed.' }
