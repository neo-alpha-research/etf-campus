...
export function classifyMarketRegime(payload: MarketBriefingPayload): MarketRegime {
  ...
  switch (code) {
    ...
    case "EXTREME_SURGE":
      ...
      slide6Block1Desc = `상승 종목(${up}개)이 압도적인 서지 랠리를 기록하며 시장 전반에 폭발적인 자금 유입 전개. 단기 이격 과열에 따른 차익실현 물량 점검 필요.`;
      captionOpening = `어제 국내 증시는 코스피가 무려 ${kospiSign}${kospi.toFixed(2)}% 폭등하며 이례적인 초급등 서지 국면을 연출했습니다. 일반 ETF 시장에서도 상승 ${up}개로 폭발적인 매수세가 시장을 장악했습니다.`;
      captionMarketSummary = `기관과 외인의 강력한 숏스퀴즈와 패시브 매수세가 맞물리며 지수 대형주와 주도 테마 전반으로 폭발적인 수급 쏠림이 나타났습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 섹터가 폭등세를 견인하며 지수 상승을 주도한 반면, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 테마조차 하방 경직성을 유지했습니다.`;
      captionWatchPoint = `초급등 국면에서는 추격 매수의 실익보다 단기 이격 과열에 따른 변동성에 유의하며, 보유 비중의 일부를 분할 익절하거나 리밸런싱하는 전략이 유효합니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 폭등하며 시장이 엄청난 상승 서지를 기록했어요.`;
    ...
    case "SUPER_BULL":
      ...
      slide6Block1Desc = `상승 종목(${up}개)이 압도적인 우위를 점하며 지수 레버리지와 고베타 테마 전반으로 상방 모멘텀 급격히 가속화.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 급등하며 시장 전반에 강력한 슈퍼 랠리가 펼쳐졌습니다. 일반 ETF 시장 역시 ${up}개 종목이 상승하며 뜨거운 열기를 뿜어냈습니다.`;
      captionMarketSummary = `대형 성장주와 대표지수 ETF로 대규모 외인·기관 동반 순매수가 집중되며 지수의 상방 탄력이 급격히 확대되었습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 테마가 시장 전반의 상승 랠리를 주도했고, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 섹터는 소폭 숨을 고르는 데 그쳤습니다.`;
      captionWatchPoint = `슈퍼 랠리에서는 상방 모멘텀의 지속성을 살피되, 고베타 종목의 변동성 확대에 대비한 분산 포트폴리오 유지가 현명합니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 시원하게 뻗어나가며 강력한 슈퍼 랠리를 완성했어요.`;
    ...
    case "STRONG_BULL_HIGH":
      ...
      slide6Block1Desc = `상승 종목(${up}개)이 75% 이상을 차지하며 주도 성장주부터 낙폭과대 섹터까지 폭넓은 매수세 유입.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 큰 폭으로 오르며 시장 전반으로 온기가 빠르게 확산되었습니다. 일반 ETF 시장에서도 상승 ${up}개로 확고한 매수 우위가 나타났습니다.`;
      captionMarketSummary = `외인과 기관의 강력한 동반 순매수가 유입되며 대형주뿐만 아니라 중소형 성장 테마군까지 폭넓은 수급 유입이 전개되었습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 섹터가 견조한 매수세를 흡수하며 강세를 보였고, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 테마는 상대적으로 제한된 움직임을 나타냈습니다.`;
      captionWatchPoint = `광범위한 상승장에서는 주도 테마의 거래대금 회전율과 실질 자금 유입의 연속성을 체크하는 것이 유리합니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 상승하며 시장 곳곳으로 따뜻한 온기가 확산되었어요.`;
    ...
    case "MODERATE_BULL":
      ...
      slide6Block1Desc = `상승 종목(${up}개)이 우위를 유지하며 주도 테마를 중심으로 지수 상방 탄력이 안정적으로 이어지는 흐름 전개.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 오르며 견조한 상승세를 이어갔습니다. 일반 ETF 시장에서도 상승 ${up}개로 안정적인 매수 우위 장세가 펼쳐졌습니다.`;
      captionMarketSummary = `주도 테마와 대표지수 ETF로 꾸준한 자금 유입이 지속되며 시장 전반의 상승 탄력이 탄탄하게 유지되었습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 섹터가 안정적인 오름세로 시장을 이끌었고, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 테마는 보합권에서 매물을 소화했습니다.`;
      captionWatchPoint = `견조한 상승 국면에서는 시장의 지수 상승폭과 더불어 자금이 집중되는 주도 테마의 이익 모멘텀을 확인하는 것이 좋습니다.`;
      threadsOpening = `어제 코스피는 ${kospiSign}${kospi.toFixed(2)}% 견조하게 오르며 안정적인 매수세를 유지했어요.`;
    ...
    case "TIGHT_BULL_SIDEWAYS":
      ...
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 소폭 오르며 강보합권에서 방향성을 탐색했습니다. 일반 ETF 시장은 상승 ${up}개, 하락 ${down}개로 팽팽한 힘겨루기가 이어졌습니다.`;
      captionMarketSummary = `지수 자체의 등락은 제한적이었으나, 뚜렷한 모멘텀을 가진 개별 테마로 스마트머니의 선별적 분할 매수가 유입되었습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 테마가 선별적인 매수세를 이끌어낸 반면, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 섹터는 매물 소화 과정을 거쳤습니다.`;
      captionWatchPoint = `강보합 국면에서는 섣부른 방향성 추종보다 거래대금이 유지되는 핵심 테마와 실질 순유입 종목의 수급을 확인하는 전략이 적합합니다.`;
      threadsOpening = `어제 코스피는 ${kospiSign}${kospi.toFixed(2)}% 소폭 오르며 강보합권에서 방향성을 모색했어요.`;
    ...
    case "TIGHT_BEAR_SIDEWAYS":
      ...
      slide6Block1Desc = `하락 종목(${down}개)이 미세하게 우세했으나 전반적으로 횡보권에서 개별 테마 중심의 차별화 장세 형성.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 약보합권에 머물며 차분한 숨고르기 양상을 보였습니다. 일반 ETF 시장에서도 하락 ${down}개로 소폭 매물이 우세했습니다.`;
      captionMarketSummary = `지수의 하방 압력은 제한적인 가운데, 단기 급등 테마의 차익 매물 출회와 방어 섹터로의 순환매가 엇갈렸습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 테마가 지수 약세에도 플러스를 지켰으며, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 섹터는 소폭 조정을 받았습니다.`;
      captionWatchPoint = `약보합 횡보장에서는 무리한 매매를 줄이고, 자산배분 차원에서 포트폴리오의 변동성 노출을 점검하는 것이 유리합니다.`;
      threadsOpening = `어제 코스피는 ${kospiSign}${kospi.toFixed(2)}% 약보합권에서 차분하게 숨을 골랐어요.`;
    ...
    case "MODERATE_BEAR":
      ...
      slide6Block1Desc = `경기민감 대형주 중심으로 하락 종목(${down}개)이 확대되었으나, 배당·채권형 ETF가 지수 대비 하방 충격을 완충.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 하락하며 본격적인 조정 국면에 진입했습니다. 일반 ETF 시장에서도 ${down}개 종목에 매물이 출회되었습니다.`;
      captionMarketSummary = `경기민감 대형주를 중심으로 매도 압력이 가중되었으나, 고배당 및 채권형 등 방어적 자산군으로 피신성 자금이 유입되며 지수 낙폭을 완충했습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 등 방어 테마가 선방한 반면, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 섹터는 매도세가 확대되었습니다.`;
      captionWatchPoint = `본격 조정 국면에서는 단기 저가 매수보다 경기 방어 자산의 비중과 주요 수급 주체의 이탈 여부를 면밀히 관찰하는 것이 안전합니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 하락하며 시장 전반에 조정의 골이 깊어졌어요.`;
    ...
    case "HEAVY_DROP":
      ...
      slide6Block1Desc = `전방위적인 매도 공세로 하락 종목(${down}개)이 속출하는 가운데, 초단기 채권 및 인버스형 자산으로의 피신성 자금 이동 가속화.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 급락하며 시장 주요 지지선을 위협하는 거센 하락 충격을 겪었습니다. 일반 ETF 시장에서도 ${down}개 종목이 일제히 하락세를 면치 못했습니다.`;
      captionMarketSummary = `외인과 기관의 동반 매도가 쏟아지며 대부분의 섹터가 약세를 보였으며, 극히 일부의 초단기 파킹형 ETF만이 피신처 역할을 수행했습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 등 극소수 테마만 버텨냈을 뿐, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 섹터를 비롯한 대다수 종목이 큰 폭의 조정을 받았습니다.`;
      captionWatchPoint = `하락 충격이 거센 구간에서는 섣부른 물타기나 저가 매수를 지양하고, 변동성 지수와 외국인 수급의 매도세 진정 여부를 차분히 확인해야 합니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 크게 밀리며 시장 전반에 거센 하락 충격이 전해졌어요.`;
    ...
    default:
      ...
      slide6Block1Desc = `하락 종목(${down}개)이 시장의 85% 이상을 차지하며 전방위 패닉 투매 발생. 무리한 포지션 진입을 금지하고 리스크 방어선 점검 필수.`;
      captionOpening = `어제 국내 증시는 코스피가 ${kospiSign}${kospi.toFixed(2)}% 폭락하며 시장 전반에 걸쳐 극단적인 변동성과 패닉성 투매가 출회되었습니다. 일반 ETF 시장 역시 ${down}개 종목이 하락하며 거센 충격을 받았습니다.`;
      captionMarketSummary = `시스템 리스크 우려로 전 섹터에 걸쳐 무차별적인 매물이 쏟아졌으며, 자금은 초단기 현금성 MMF 및 파킹형 ETF로 긴급 대피하는 모습을 나타냈습니다.`;
      captionThemeAnalysis = `${topThemeName}(${topThemeRet > 0 ? "+" : ""}${topThemeRet.toFixed(2)}%) 등 일부 헷지형 자산을 제외한 전 섹터가 급락세를 보였으며, ${bottomThemeName}(${bottomThemeRet.toFixed(2)}%) 테마는 극심한 낙폭을 기록했습니다.`;
      captionWatchPoint = `극단적 패닉 장세에서는 심리적 공포에 의한 투매 동참도, 성급한 바닥 낚시도 모두 위험합니다. 환율 급등세 진정과 매도 호가 공백 해소를 기다리는 보수적 관망이 필수적입니다.`;
      threadsOpening = `어제 코스피가 ${kospiSign}${kospi.toFixed(2)}% 폭락하며 시장이 극단적인 패닉 투매를 겪었습니다.`;
