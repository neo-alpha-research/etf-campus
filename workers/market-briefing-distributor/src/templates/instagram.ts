...
export function generateInstagramCarousel(
  ...
  const slide1Svg = `
    <svg width="1080" height="1350" viewBox="0 0 1080 1350" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ETF 데일리 마켓 브리핑 - 표지 및 핵심 펄스">
      ...
      <!-- Main Hero Hook Card (y=122, h=170) -->
      <g transform="translate(70, 122)" filter="url(#softShadow)">
        <rect width="940" height="170" rx="24" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="2"/>
        
        <!-- 헤드라인 1: 빅 넘버 대비 (48px 대형 볼드) -->
        <text x="35" y="68" fill="#0F172A" font-size="48" font-weight="900" letter-spacing="-1.2">
        ...
        <text x="35" y="126" fill="#047857" font-size="30" font-weight="900" letter-spacing="-0.6">
          &apos;${escapeXml(cleanTopThemeName)}&apos; ${themeVerb} 속 &apos;${escapeXml(cleanInflowBannerName)}&apos; 수급 집중
        </text>
      </g>

      <!-- Pulse 1: Market Temperature (y=312, h=202) -->
      <g transform="translate(70, 312)" filter="url(#cardShadow)">
        <rect width="940" height="202" rx="24" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="1.6"/>
        <text x="35" y="42" fill="#1E293B" font-size="23" font-weight="900">1. 시장 체온 &amp; 3대 지수 비교</text>
        
        <rect x="605" y="16" width="300" height="40" rx="12" fill="#F8FAFC" stroke="#E2E8F0" stroke-width="1.2"/>
        <text x="755" y="43" font-size="18" font-weight="900" text-anchor="middle">
        ...
        <g transform="translate(35, 76)">
          <!-- KOSPI -->
          <rect x="0" y="0" width="275" height="98" rx="15" fill="${kospi >= 0 ? '#FEF2F2' : '#EFF6FF'}" stroke="${kospi >= 0 ? '#FECACA' : '#BFDBFE'}" stroke-width="1.2"/>
          <text x="22" y="58" fill="${kospi >= 0 ? '#991B1B' : '#1E40AF'}" font-size="21" font-weight="800">KOSPI</text>
          <text x="252" y="61" fill="${kospiColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${kospiSign}${kospi.toFixed(2)}%</text>

          <!-- KOSDAQ -->
          <rect x="297" y="0" width="275" height="98" rx="15" fill="${kosdaq >= 0 ? '#FEF2F2' : '#EFF6FF'}" stroke="${kosdaq >= 0 ? '#FECACA' : '#BFDBFE'}" stroke-width="1.2"/>
          <text x="319" y="58" fill="${kosdaq >= 0 ? '#991B1B' : '#1E40AF'}" font-size="21" font-weight="800">KOSDAQ</text>
          <text x="549" y="61" fill="${kosdaqColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${kosdaqSign}${kosdaq.toFixed(2)}%</text>

          <!-- 일반 ETF -->
          <rect x="595" y="0" width="275" height="98" rx="15" fill="#F0FDF4" stroke="#BBF7D0" stroke-width="1.5"/>
          <text x="617" y="58" fill="#15803D" font-size="21" font-weight="900">일반 ETF</text>
          <text x="847" y="61" fill="${etfColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${etfSign}${etfReturn.toFixed(2)}%</text>
        </g>
      </g>

      <!-- Pulse 2: Long/Short Themes (y=534, h=202) -->
      <g transform="translate(70, 534)" filter="url(#cardShadow)">
        <rect width="940" height="202" rx="24" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="1.6"/>
        <text x="35" y="42" fill="#0F172A" font-size="23" font-weight="900">2. 오늘의 극과 극 테마</text>
        
        <rect x="705" y="16" width="200" height="40" rx="12" fill="#FFF7ED" stroke="#FDBA74" stroke-width="1.2"/>
        <text x="805" y="42" fill="#C2410C" font-size="17" font-weight="900" text-anchor="middle">테마 격차 ${themeGap}%p</text>

        <!-- Left: ▲ 상위 1위 카드 -->
        <g transform="translate(35, 72)">
          <rect width="420" height="104" rx="16" fill="#FEF2F2" stroke="#FCA5A5" stroke-width="1.2"/>
          <text x="22" y="34" fill="#B42318" font-size="16" font-weight="900">▲ 상위 1위</text>
          <text x="22" y="72" fill="#0F172A" font-size="24" font-weight="900">${escapeXml(topTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim())}</text>
          <text x="400" y="68" fill="#D92D20" font-size="38" font-weight="900" text-anchor="end" class="tabular">${(topTheme.cappedAumWeightedReturnPct ?? 0) > 0 ? "+" : ""}${(topTheme.cappedAumWeightedReturnPct ?? 0).toFixed(2)}%</text>
        </g>

        <!-- Right: ▼ 하위 1위 카드 -->
        <g transform="translate(485, 72)">
          <rect width="420" height="104" rx="16" fill="#EFF6FF" stroke="#93C5FD" stroke-width="1.2"/>
          <text x="22" y="34" fill="#175CD3" font-size="16" font-weight="900">▼ 하위 1위</text>
          <text x="22" y="72" fill="#0F172A" font-size="${bottomTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim().length > 14 ? 19 : (bottomTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim().length > 11 ? 21 : 24)}" font-weight="900">${escapeXml(bottomTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim())}</text>
          <text x="400" y="68" fill="#175CD3" font-size="38" font-weight="900" text-anchor="end" class="tabular">${(bottomTheme.cappedAumWeightedReturnPct ?? 0) > 0 ? "+" : ""}${(bottomTheme.cappedAumWeightedReturnPct ?? 0).toFixed(2)}%</text>
        </g>
      </g>

      <!-- Pulse 3: Top Inflow (y=756, h=202) -->
      <g transform="translate(70, 756)" filter="url(#cardShadow)">
        <rect width="940" height="202" rx="24" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="1.6"/>
        <text x="35" y="42" fill="#1E293B" font-size="23" font-weight="900">3. 실질 자금 순유입 1위</text>
        
        <rect x="755" y="16" width="150" height="40" rx="12" fill="#F1F5F9" stroke="#CBD5E1" stroke-width="1.2"/>
        <text x="830" y="42" fill="#1E293B" font-size="16" font-weight="900" text-anchor="middle">기관·외국인 합산</text>

        <!-- Main Inflow Row (No Parentheses) -->
        <g transform="translate(35, 70)">
          <rect width="870" height="70" rx="15" fill="#F0FDF4" stroke="#BBF7D0" stroke-width="1.4"/>
          <circle cx="35" cy="35" r="17" fill="#10B981"/>
          <text x="35" y="41" fill="#FFFFFF" font-size="16" font-weight="900" text-anchor="middle">1</text>
          <text x="70" y="43" fill="#0F172A" font-size="25" font-weight="900">
            ${escapeXml(cleanInflowBannerName)} <tspan fill="#64748B" font-size="17" font-weight="700">· ${escapeXml(topInflow.ticker)}</tspan>
          </text>
          <text x="850" y="46" fill="#047857" font-size="38" font-weight="900" text-anchor="end" class="tabular">+${(topInflow.inflow ?? 0).toLocaleString()}억원</text>
        </g>

        <text x="35" y="172" fill="#334155" font-size="18" font-weight="800">
          상위 5종목 총 ${top5InflowSum.toLocaleString()}억원 순유입 · 상세 순위는 4페이지에서 확인
        </text>
      </g>

      <!-- Bottom Swipe CTA (y=980, h=92) -->
      <g transform="translate(70, 980)" filter="url(#softShadow)">
        <rect width="940" height="92" rx="24" fill="url(#brandGrad)"/>
        <text x="430" y="57" fill="#FFFFFF" font-size="24" font-weight="900" text-anchor="middle" letter-spacing="-0.5">
          👉 옆으로 넘겨 테마·수급·괴리율 상세 분석
        </text>
        <rect x="805" y="26" width="90" height="40" rx="12" fill="#064E3B"/>
        <text x="850" y="52" fill="#A7F3D0" font-size="17" font-weight="900" text-anchor="middle" class="tabular">1 / ${totalSlides}</text>
      </g>
  ...
  const slide2Svg = `
    <svg width="1080" height="1350" viewBox="0 0 1080 1350" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ETF 데일리 마켓 브리핑 - 주도 테마 TOP 3 vs 부진 테마">
      ...
      <g transform="translate(70, 56)">
        <text x="0" y="36" fill="#0F172A" font-size="36" font-weight="900">오늘 시장 주도/부진 테마 TOP 3</text>
        <text x="0" y="70" fill="#64748B" font-size="16" font-weight="600">※ 테마별 AUM 가중수익률 기준 상위/하위 랭킹</text>
        <rect x="830" y="6" width="110" height="44" rx="14" fill="#F1F5F9" stroke="#CBD5E1" stroke-width="1.5"/>
        <text x="885" y="34" fill="#475569" font-size="18" font-weight="900" text-anchor="middle" class="tabular">2 / ${totalSlides}</text>
      </g>

      <!-- Summary Banner (y=138, h=116) -->
      <g transform="translate(70, 138)" filter="url(#cardShadow)">
        <rect width="940" height="116" rx="22" fill="#FFFBEB" stroke="#FDE68A" stroke-width="1.5"/>
        ...
        <text x="170" y="43" fill="#0F172A" font-size="29" font-weight="900">&apos;${escapeXml(topTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim())}&apos; ${(topTheme.cappedAumWeightedReturnPct ?? 0) > 0 ? "주도" : "선방"} vs &apos;${escapeXml(bottomTheme.peerGroup.replace(/\s*\([^)]*\)/g, '').trim())}&apos; 조정</text>
      ...
      <!-- Panel 1: Top 3 Leaders (▲ 상위 Top 3) (y=274, h=415) -->
      <g transform="translate(70, 274)" filter="url(#cardShadow)">
        <rect width="940" height="415" rx="24" fill="#FFFFFF" stroke="#FECDCA" stroke-width="1.5"/>
        <rect x="0" y="0" width="940" height="52" rx="24" fill="#FEF3F2"/>
        <text x="35" y="34" fill="#B42318" font-size="22" font-weight="900">▲ 상위 Top 3 주도 테마</text>
        ...
        ${winners.map((w, idx) => {
          ...
          return `
          <g transform="translate(35, ${70 + idx * 110})">
            <rect width="870" height="98" rx="18" fill="#FAFDF4" stroke="#D7EABB" stroke-width="1.5"/>
            <circle cx="45" cy="49" r="22" fill="${idx === 0 ? '#D92D20' : '#FEE4E2'}"/>
            <text x="45" y="57" fill="${idx === 0 ? '#FFFFFF' : '#D92D20'}" font-size="20" font-weight="900" text-anchor="middle">${idx + 1}</text>
            <text x="85" y="42" fill="#0F172A" font-size="${nameFs}" font-weight="900">${escapeXml(cleanName)}</text>
            <text x="85" y="74" fill="#64748B" font-size="17" font-weight="600">총 ${w.etfCount}개 ETF 구성 · 자산군: ${escapeXml(w.assetClass || "국내주식")}</text>
            <text x="840" y="58" fill="${retColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${retSign}${ret.toFixed(2)}%</text>
          </g>
        `;}).join("")}
      </g>

      <!-- Panel 2: Bottom 3 Laggards (▼ 하위 Worst 3) (y=709, h=415) -->
      <g transform="translate(70, 709)" filter="url(#cardShadow)">
        <rect width="940" height="415" rx="24" fill="#FFFFFF" stroke="#BFDBFE" stroke-width="1.5"/>
        <rect x="0" y="0" width="940" height="52" rx="24" fill="#EFF6FF"/>
        <text x="35" y="34" fill="#1D4ED8" font-size="22" font-weight="900">▼ 하위 Worst 3 부진 테마</text>
        ...
        ${losers.map((l, idx) => {
          ...
          return `
          <g transform="translate(35, ${70 + idx * 110})">
            <rect width="870" height="98" rx="18" fill="#F8FAFC" stroke="#E2E8F0" stroke-width="1.5"/>
            <circle cx="45" cy="49" r="22" fill="${idx === 0 ? '#175CD3' : '#DBEAFE'}"/>
            <text x="45" y="57" fill="${idx === 0 ? '#FFFFFF' : '#175CD3'}" font-size="20" font-weight="900" text-anchor="middle">${idx + 1}</text>
            <text x="85" y="42" fill="#0F172A" font-size="${nameFs}" font-weight="900">${escapeXml(cleanName)}</text>
            <text x="85" y="74" fill="#64748B" font-size="17" font-weight="600">총 ${l.etfCount}개 ETF 구성 · 자산군: ${escapeXml(l.assetClass || "해외주식")}</text>
            <text x="840" y="58" fill="${retColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${retSign}${ret.toFixed(2)}%</text>
          </g>
        `;}).join("")}
      </g>
  ...
  const slide3Svg = `
    <svg width="1080" height="1350" viewBox="0 0 1080 1350" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ETF 데일리 마켓 브리핑 - 자산군별 성과 및 분산 기여도">
      ...
      <g transform="translate(70, 56)">
        <text x="0" y="36" fill="#0F172A" font-size="36" font-weight="900">자산군별 수익률/기여도</text>
        <text x="0" y="70" fill="#64748B" font-size="16" font-weight="600">※ 자산군별 당일 가중수익률, 순자산 비중 및 시장 기여도 현황입니다.</text>
        <rect x="830" y="6" width="110" height="44" rx="14" fill="#F1F5F9" stroke="#CBD5E1" stroke-width="1.5"/>
        <text x="885" y="34" fill="#475569" font-size="18" font-weight="900" text-anchor="middle" class="tabular">3 / ${totalSlides}</text>
      </g>

      <!-- Summary Box (y=138, h=114) -->
      <g transform="translate(70, 138)" filter="url(#cardShadow)">
      ...
      <!-- 6 Asset Classes List (y=274, step=144, h=132) -->
      <g transform="translate(70, 274)">
        ${assetClasses.slice(0, 6).map((ac, idx) => {
          ...
          return `
            <g transform="translate(0, ${idx * 144})" filter="url(#cardShadow)">
              <rect width="940" height="132" rx="20" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="1.5"/>
              <text x="35" y="44" fill="#0F172A" font-size="30" font-weight="900">${escapeXml(ac.assetClass)}</text>
              <text x="35" y="78" fill="#334155" font-size="20" font-weight="700">
                순자산 <tspan font-weight="900" fill="#0F172A">${aumJo}조원</tspan> · 비중 <tspan font-weight="900" fill="#2E6819">${share.toFixed(1)}%</tspan>
              </text>
              
              <rect x="35" y="98" width="380" height="10" rx="5" fill="#F1F5F9"/>
              <rect x="35" y="98" width="${Math.min(380, share * 3.8)}" height="10" rx="5" fill="#2E6819"/>
              
              <rect x="490" y="14" width="415" height="104" rx="15" fill="#F8FAFC" stroke="#E2E8F0" stroke-width="1"/>
              <text x="515" y="48" fill="#475569" font-size="19" font-weight="800">당일 가중수익률</text>
              <text x="880" y="50" fill="${retColor}" font-size="38" font-weight="900" text-anchor="end" class="tabular">${retSign}${ret.toFixed(2)}%</text>
              
              <text x="515" y="88" fill="#64748B" font-size="17" font-weight="700">시장 기여도</text>
              <text x="880" y="90" fill="${retColor}" font-size="21" font-weight="900" text-anchor="end" class="tabular">${contribSign}${contribution}%p</text>
            </g>
          `;
        }).join("")}
      </g>
  ...
  const slide4Svg = `
    <svg width="1080" height="1350" viewBox="0 0 1080 1350" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ETF 데일리 마켓 브리핑 - 스마트머니 실질 순유입 TOP 5">
      ...
      <g transform="translate(70, 56)">
        <text x="0" y="36" fill="#0F172A" font-size="38" font-weight="900">실질 자금 순유입 TOP 5</text>
        <text x="0" y="70" fill="#64748B" font-size="16" font-weight="600">※ 발행좌수 증감으로 산출된 기관·외국인의 실질 자금 순유입액</text>
        <rect x="830" y="6" width="110" height="44" rx="14" fill="#F1F5F9" stroke="#CBD5E1" stroke-width="1.5"/>
        <text x="885" y="34" fill="#475569" font-size="18" font-weight="900" text-anchor="middle" class="tabular">4 / ${totalSlides}</text>
      </g>

      <!-- Summary Banner (y=138, h=100) -->
      <g transform="translate(70, 138)" filter="url(#cardShadow)">
        <rect width="940" height="100" rx="22" fill="#FFF1F2" stroke="#FECDD3" stroke-width="1.5"/>
        <rect x="30" y="16" width="125" height="34" rx="10" fill="#FFE4E6" stroke="#FDA4AF" stroke-width="1.2"/>
        <text x="92" y="39" fill="#BE123C" font-size="17" font-weight="900" text-anchor="middle">수급 핵심</text>
        <text x="170" y="40" fill="#0F172A" font-size="${slide4TitleFs}" font-weight="900">${slide4BannerTitle}</text>
        <text x="30" y="80" fill="#334155" font-size="20" font-weight="700">
          ${escapeXml(slide4BannerDesc)}
        </text>
      </g>

      <!-- TOP 5 Inflow Ranking Cards (y=260, step=174, h=164) -->
      <g transform="translate(70, 260)">
        ${topInflows.slice(0, 5).map((item, idx) => {
          const inflowJo = item.inflow ? item.inflow.toLocaleString() : "1,000";
          const isTop = idx === 0;
          return `
            <g transform="translate(0, ${idx * 174})" filter="url(#cardShadow)">
              <rect width="940" height="164" rx="22" fill="${isTop ? '#FFF8F8' : '#FFFFFF'}" stroke="${isTop ? '#FCA5A5' : '#E2E8F0'}" stroke-width="${isTop ? '2' : '1.5'}"/>
              ${isTop ? '<rect x="0" y="0" width="8" height="164" rx="4" fill="#D92D20"/>' : ''}

              <!-- 순위 뱃지 -->
              <circle cx="58" cy="82" r="26" fill="${isTop ? '#D92D20' : '#F1F5F9'}" ${!isTop ? 'stroke="#E2E8F0" stroke-width="1.5"' : ''}/>
              <text x="58" y="91" fill="${isTop ? '#FFFFFF' : '#475569'}" font-size="22" font-weight="900" text-anchor="middle">${idx + 1}</text>

              <!-- ETF명 (말줄임 없이 풀네임 노출 + 동적 폰트 스케일링) -->
              <text x="100" y="66" fill="#0F172A" font-size="${item.name.length > 22 ? 21 : (item.name.length > 18 ? 23 : 26)}" font-weight="900">${escapeXml(item.name)}</text>

              <!-- 티커 -->
              <rect x="100" y="82" width="76" height="26" rx="6" fill="#F1F5F9" stroke="#E2E8F0" stroke-width="1"/>
              <text x="138" y="100" fill="#64748B" font-size="13.5" font-weight="700" text-anchor="middle" class="tabular">${item.ticker}</text>

              <!-- 테마 태그 -->
              <rect x="186" y="82" width="125" height="26" rx="6" fill="${isTop ? '#FFE4E6' : '#F8FAFC'}" stroke="${isTop ? '#FDA4AF' : '#E2E8F0'}" stroke-width="1"/>
              <text x="248" y="100" fill="${isTop ? '#BE123C' : '#64748B'}" font-size="13.5" font-weight="800" text-anchor="middle">${escapeXml(item.theme || "핵심ETF")}</text>

              <!-- 순유입 금액 -->
              <text x="912" y="74" fill="${isTop ? '#D92D20' : '#1E293B'}" font-size="40" font-weight="900" text-anchor="end" class="tabular">+${inflowJo}<tspan font-size="22" font-weight="700" fill="${isTop ? '#BE123C' : '#64748B'}">억원</tspan></text>
              <text x="912" y="106" fill="${isTop ? '#E11D48' : '#64748B'}" font-size="15.5" font-weight="800" text-anchor="end">${isTop ? '당일 최대 실질 순유입' : '순유입 상위 종목'}</text>
            </g>
          `;
        }).join("")}
      </g>
  ...
  const slide5Svg = `
    <svg width="1080" height="1350" viewBox="0 0 1080 1350" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ETF 데일리 마켓 브리핑 - 3대 마켓 체크리스트 및 관전 포인트">
      ...
      <!-- Header -->
      <g transform="translate(70, 56)">
        <text x="0" y="36" fill="#1E3A8A" font-size="40" font-weight="900" letter-spacing="-1.0">오늘 시장 총정리 &amp; 핵심 전략</text>
        <text x="0" y="70" fill="#64748B" font-size="16" font-weight="600">※ 3대 핵심 축으로 요약하는 시장 진단과 실전 투자 대응 가이드</text>
        <rect x="830" y="6" width="110" height="44" rx="14" fill="#F1F5F9" stroke="#CBD5E1" stroke-width="1.5"/>
        <text x="885" y="34" fill="#475569" font-size="18" font-weight="900" text-anchor="middle" class="tabular">${totalSlides} / ${totalSlides}</text>
      </g>

      <!-- 1. CARD 01: [시장 진단] (Market Pulse & Regime) (y=138, h=280) -->
      <g transform="translate(70, 138)" filter="url(#cardShadow)">
        <rect width="940" height="280" rx="24" fill="#FFFFFF" stroke="#BBF7D0" stroke-width="1.8"/>
        <rect x="0" y="0" width="8" height="280" rx="4" fill="#10B981"/>
        ...
        <!-- 1행: 핵심 팩트 (26px 대형 볼드) -->
        <text x="35" y="138" fill="#1E293B" font-size="26" font-weight="900">
          • 상승 종목 ${up}개 우위로 시장 매수 심리 전반 회복
        </text>

        <!-- 2행: 실전 대응 액션 밴드 (23px 볼드) -->
        <rect x="35" y="180" width="870" height="72" rx="14" fill="#F0FDF4" stroke="#BBF7D0" stroke-width="1.2"/>
        <rect x="52" y="197" width="95" height="38" rx="9" fill="#DCFCE7"/>
        <text x="99" y="222" fill="#15803D" font-size="16.5" font-weight="900" text-anchor="middle">대응 요약</text>
        <text x="162" y="223" fill="#166534" font-size="23" font-weight="800">대형주 및 핵심 성장 섹터 중심의 안정적 상방 탄력 유효</text>
      </g>

      <!-- 2. CARD 02: [테마 순환] (Theme Rotation & Divergence) (y=440, h=280) -->
      <g transform="translate(70, 440)" filter="url(#cardShadow)">
        <rect width="940" height="280" rx="24" fill="#FFFFFF" stroke="#FECDD3" stroke-width="1.8"/>
        <rect x="0" y="0" width="8" height="280" rx="4" fill="#F43F5E"/>
        ...
        <!-- 2행: 실전 대응 액션 밴드 (23px 볼드) -->
        <rect x="35" y="180" width="870" height="72" rx="14" fill="#FFF1F2" stroke="#FECDD3" stroke-width="1.2"/>
        <rect x="52" y="197" width="95" height="38" rx="9" fill="#FFE4E6"/>
        <text x="99" y="222" fill="#BE123C" font-size="16.5" font-weight="900" text-anchor="middle">전략 요약</text>
        <text x="162" y="223" fill="#9F1239" font-size="23" font-weight="800">단기 급등 테마 무리한 추격 매수 자제 및 분할 익절·리밸런싱</text>
      </g>

      <!-- 3. CARD 03: [대응 전략] (Smart Money & Action Strategy) (y=742, h=280) -->
      <g transform="translate(70, 742)" filter="url(#cardShadow)">
        <rect width="940" height="280" rx="24" fill="#FFFFFF" stroke="#BFDBFE" stroke-width="1.8"/>
        <rect x="0" y="0" width="8" height="280" rx="4" fill="#3B82F6"/>
        ...
        <!-- 2행: 실전 대응 액션 밴드 (23px 볼드) -->
        <rect x="35" y="180" width="870" height="72" rx="14" fill="#EFF6FF" stroke="#BFDBFE" stroke-width="1.2"/>
        <rect x="52" y="197" width="95" height="38" rx="9" fill="#DBEAFE"/>
        <text x="99" y="222" fill="#1D4ED8" font-size="16.5" font-weight="900" text-anchor="middle">실전 조언</text>
        <text x="162" y="223" fill="#1E40AF" font-size="23" font-weight="800">단기 시세 추종 지양, 안전자산 완충력 확보하는 균형 전략 유효</text>
      </g>

      <!-- Bottom KRX Notice Banner (y=1050, h=58) -->
      <g transform="translate(70, 1050)">
        <rect width="940" height="58" rx="16" fill="#F8FAFC" stroke="#CBD5E1" stroke-width="1.2"/>
        <text x="470" y="37" fill="#475569" font-size="19" font-weight="800" text-anchor="middle">
